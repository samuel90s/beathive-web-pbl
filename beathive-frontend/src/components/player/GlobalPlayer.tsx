...
export default function GlobalPlayer() {
  ...
  // Ganti track saat currentTrack berubah
  useEffect(() => {
    ...
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-gray-100 px-4 py-3">
      ...
      <div className="max-w-7xl mx-auto flex items-center gap-4">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <button onClick={stop} className="w-8 h-8 rounded-full text-gray-400 hover:text-gray-600 flex items-center justify-center">
        ...
        <div className="flex-shrink-0 min-w-0 w-44">
          ...
          <p className="text-xs text-gray-400">{currentTrack.category.name} · Preview 30s</p>
        </div>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          {currentTrack.isFree ? (
            <button
              onClick={() => download(currentTrack.id)}
